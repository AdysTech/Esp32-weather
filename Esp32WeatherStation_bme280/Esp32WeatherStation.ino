...
void setup()
{
  ...
  // ESP32 0x0460 chip read 1061mV
  // Configure ADC
  adc1_config_width(ADC_WIDTH_12Bit);
  adc1_config_channel_atten(ADC1_CHANNEL_6, ADC_ATTEN_11db);

  // Calculate ADC characteristics i.e. gain and offset factors
  esp_adc_cal_get_characteristics(1061, ADC_ATTEN_DB_11, ADC_WIDTH_BIT_12, &characteristics);
  ...
  if (vBat > 3.4)
  {
    ...
    if (b)
    {
      ...
      sprintf(value, "weather,board=0x%04X%08X,sensor=bme280,region=outside,variable=Temperature,unit=Celsius value=%f %d", (uint16_t)(chipid >> 32), (uint32_t)chipid, bme.getTemperature(), now);
      PostToInflux(value);
      sprintf(value, "weather,board=0x%04X%08X,sensor=bme280,region=outside,variable=Relative_Humidity,unit=Percentage value=%f %d", (uint16_t)(chipid >> 32), (uint32_t)chipid, bme.getHumidity(), now);
      PostToInflux(value);
      sprintf(value, "weather,board=0x%04X%08X,sensor=bme280,region=outside,variable=Pressure,unit=Hectopascal value=%f %d", (uint16_t)(chipid >> 32), (uint32_t)chipid, bme.getPressure(), now);
      PostToInflux(value);
      sprintf(value, "weather,board=0x%04X%08X,sensor=bme280,region=outside,variable=Dew_Point,unit=Celsius value=%f %d", (uint16_t)(chipid >> 32), (uint32_t)chipid, bme.getDewPoint(), now);
      PostToInflux(value);
      sprintf(value, "weather,board=0x%04X%08X,sensor=bme280,region=outside,variable=Heat_Index,unit=Celsius value=%f %d", (uint16_t)(chipid >> 32), (uint32_t)chipid, bme.getHeatIndex(), now);
      PostToInflux(value);
    }
